...
class Hassle
  ...
  def call(env)
    ...
    # elsif @compiler.images.include?(path)
    #   image = path.split('/').last
    #   image_path = File.join(@compiler.compile_location, path)
    # 
    #   # Read the contents of the image
    #   content = File.read(image_path)
    #   length = "".respond_to?(:bytesize) ? content.bytesize.to_s : content.size.to_s
    #   [200, {'Content-Type' => 'image/png', 'Content-Length' => length}, [content]]
    else
...
class Hassle::Compiler
  ...
  def normalize
    ...
    Compass.configuration.images_path = compile_location('images') if !Compass.nil?
  end
  ...
  def prepare
    ...
    FileUtils.mkdir_p(Compass.configuration.images_path)
  end
  ...
  # def images
  #   options[:images_path].to_a.map do |location|
  #     Dir[File.join(location.last, "**", "sprites_*.png")].map { |css| css.gsub(compile_location, "/") }
  #   end.flatten.sort
  # end
